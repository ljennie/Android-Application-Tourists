...
public class CommentAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    @Override
    public int getItemViewType(int position) {//getItemViewType帮助告诉我们是显示event还是comment
        return position == 0 ? TYPE_EVENT : TYPE_COMMENT;
    }

    @Override
    public int getItemCount() {
        return commentList.size() + 1;//因为有个event数量所以要加一
    }

    // 创建viewholder到显示viewholder有三个过程，定义viewholder, 创建viewholder, bundler viewholder
    public class EventViewHolder extends RecyclerView.ViewHolder {
        // 显示event的具体信息，并将他们放在viewholder里面
    ...
    public class CommentViewHolder extends RecyclerView.ViewHolder {
        // 显示comment的具体信息，并将他们放在viewholder里面
    ...
    // 创建viewholder
    //viewType：position对应的viewtype，有0和1，这样adapter才知道在第几行显示怎么样的layout
    @Override
    ...
    //onBindViewHolder：将创建出来的viewholder显示出来
    @Override
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        switch (holder.getItemViewType()) {
            case TYPE_EVENT:
                EventViewHolder viewHolderEvent = (EventViewHolder) holder;
                //为什么这里可以把RecyclerView.ViewHolder强制类型转换成EventViewHolder？
                //因为EventViewHolder extends RecyclerView.ViewHolder
    ...
    private void configureCommentView(final CommentViewHolder commentHolder, final int position) {
        //Why is position - 1?因为在这个list里添加了event view，所以要getcomment的时候需要position－1
